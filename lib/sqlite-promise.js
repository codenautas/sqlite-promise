    ...
    return Promises.make(function(resolve, reject){
        internal.conn.run(internal.sql, internal.data || [], function(err) {
           if(err) {
               ...
               // Atencion! esto es necesario porque this.changes solo tiene
               // resultados coherentes para UPDATE o DELETE
               //  ver: https://github.com/mapbox/node-sqlite3/wiki/API#databaserunsql-param--callback
               // entonces rowCount vale:
               //    para                           |  rowCount
               // ----------------------------------+----------------
               // update/delete                     |  this.changes
               // create/drop table/index/sequence  |  0
               //  todos los demas                  |  1
